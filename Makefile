...
install-local-provider: build $(GO)
	@mkdir -p $(DESTINATION_PREFIX)/terraform.local/local/tekton/1.0.0/$(shell $(GO) env GOOS)_$(shell $(GO) env GOARCH)
	@cp ./terraform-provider-tekton $(DESTINATION_PREFIX)/terraform.local/local/tekton/1.0.0/$(shell $(GO) env GOOS)_$(shell $(GO) env GOARCH)
	@echo "==> Installing plugin to $(DESTINATION_PREFIX)/terraform.local/local/tekton/1.0.0/$(shell $(GO) env GOOS)_$(shell $(GO) env GOARCH)"
...
test: $(GO) test-fmt
	$(GO) test ./tekton/... $(TESTARGS) -timeout=30s -parallel=4
